...
posts_list = [
    {"Название": "Пост 1", "pop": 1, "дата": "01.12.23", "лаки": 3, "коментарий": "Good"},
    {"Название": "Пост 2", "pop": 9, "дата": "03.12.23", "лаки": 15, "коментарий": "Nice"},
    {"Название": "Пост 3", "pop": 3, "дата": "04.12.23", "лаки": 7, "коментарий": "Beautifull"},
    {"Название": "Пост 4", "pop": 6, "дата": "05.12.23", "лаки": 30, "коментарий": "Very Good"},
]
...
def popular(request):
    ...
    return HttpResponse(f"""<h2>Название поста: {output_post.get('name')}</h2> 
                            <br> <h2>Значение популярности: {pop_post}</h2> <br> 
                            <h2>Количество лайков: {output_post.get('likes')}</h2> 
                            <br> <h2>Комментарий: {output_post.get('com')}</h2> 
                            <br> <h2>Дата: {output_post.get('date')}</h2> <br>""")

def new(request):
    new_post = posts_list[-1]
    return HttpResponse(f"""<h2>Название поста: {new_post.get('name')}</h2>
                        <br> <h2>Значение популярности: {new_post.get('pop')}</h2> <br> 
                        <h2>Количество лайков: {new_post.get('likes')}</h2>
                        <br> <h2>Комментарий: {new_post.get('com')}</h2> <br>
...
def index(request):
    login = request.GET.get("логин", "Ничего нет")
    password = request.GET.get("пароль", "Ничего нет")
